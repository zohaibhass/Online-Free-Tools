...
// Same comprehensive currency list as Mortgage Calculator
...
export function DiscountCalculatorTool() {
  ...
  const [usePercent, setUsePercent] = useState(true) // Toggle between % and fixed amount
  ...
  const calculate = useCallback(() => {
    ...
    setSavings(discount)
    setFinalPrice(final)
  }, [originalPrice, discountPercent, discountAmount, usePercent])

  const handleCopy = () => {
    const text = [
      `Original Price: ${currency.symbol}${originalPrice.toFixed(2)}`,
      `Discount: ${currency.symbol}${savings.toFixed(2)} (${discountPercent.toFixed(1)}%)`,
      `Final Price: ${currency.symbol}${finalPrice.toFixed(2)}`,
    ].join('\n')
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 p-3 border border-border rounded-lg bg-background text-lg"
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 py-2 rounded-lg border font-medium ${usePercent ? 'bg-primary text-primary-foreground' : 'border-border hover:bg-muted'
              }`}
          >
            Percentage (%)
          </button>
      ...
      {usePercent ? (
        <div>
          <div className="flex justify-between mb-1.5">
            ...
            <span className="font-semibold text-lg">{discountPercent}%</span>
          </div>
          <input
            type="range"
            min="0"
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 p-3 border border-border rounded-lg bg-background"
            />
          </div>
        </div>
      )}
      ...
      {/* Quick Presets */}
      <div>
        <p className="text-sm font-medium mb-2">Quick Discounts</p>
        <div className="grid grid-cols-5 gap-2">
          {[10, 20, 30, 50, 70].map((pct) => (
            <button
              ...
              className="py-2 text-sm rounded border border-border hover:bg-muted transition-colors"
            >
              {pct}%
            </button>
          ))}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 rounded-xl bg-muted border border-border">
              ...
              <p className="text-xs text-green-600 mt-1">({savingsPercent.toFixed(1)}% OFF)</p>
            </div>
